...
export async function POST(req: NextRequest) {
  try {
    ...
    if (authority === 'ONLINE' && !healthy) {
      ...
      await saveCentralBooking({ bookingId, ticketNumber, theatreId: body.theatreId, showId: body.showId, movieTitle: body.movieTitle, theatreName: body.theatreName, showTimeUtc: body.showTimeIso, showLabel: body.showLabel, totalTickets: body.seatIds.length, seats: body.seatIds, pricing: body.pricing, bookingSource: 'ONLINE_OUTAGE_MODE', bookingStatus: 'HELD', reconciliationStatus: 'NOT_SYNCED', holdId, sessionId, idempotencyKey, requestIp, sourceLabel, heldAtUtc: new Date().toISOString(), confirmedAtUtc: null });
    ...
    if (authority !== 'LOCAL') return NextResponse.json({ success: false, message: 'Online booking is not allowed right now.' }, { status: 400 });
    const localBase = theatre.localPublicUrl || process.env.LOCAL_PUBLIC_URL || '';
    const res = await fetch(`${localBase}/api/booking/hold`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ showId: body.showId, seatIds: body.seatIds, source: 'ONLINE_VIA_LOCAL', requestIp, sourceLabel }) });
    ...
    try {
      await saveCentralBooking({ bookingId: `BOOK-${crypto.randomUUID()}`, ticketNumber: `PENDING-${Date.now()}`, theatreId: body.theatreId, showId: body.showId, movieTitle: body.movieTitle, theatreName: body.theatreName, showTimeUtc: body.showTimeIso, showLabel: body.showLabel, totalTickets: body.seatIds.length, seats: body.seatIds, pricing: data.pricing || body.pricing, bookingSource: 'ONLINE_VIA_LOCAL', bookingStatus: 'HELD', reconciliationStatus: 'PENDING_SYNC', holdId: data.holdId, sessionId: data.sessionId, idempotencyKey, requestIp, sourceLabel, heldAtUtc: new Date().toISOString() });
      ...
      await fetch(`${localBase}/api/booking/release`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ holdId: data.holdId }) }).catch(() => {});

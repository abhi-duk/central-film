...
export async function readMysqlStore() {
  ...
  const db = getDb();
  const [theatreRows] = await db.query('SELECT * FROM theatres ORDER BY theatre_id');
  const [bookingRows] = await db.query('SELECT * FROM bookings ORDER BY created_at DESC');
  const [pendingRows] = await db.query('SELECT * FROM pending_transactions ORDER BY created_at DESC');
  return {

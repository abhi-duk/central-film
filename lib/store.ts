...
function toMySqlUtc(value: string | Date | null | undefined) {
...
function parseMySqlUtc(value: unknown) {
  if (!value) return null;
...
async function readMysqlStore(): Promise<Store> {
  ...
  return {
    theatres: theatreRows.map(r => ({
      ...
      lastHeartbeatAt: parseMySqlUtc(r.last_heartbeat_at),
      updatedAt: parseMySqlUtc(r.updated_at),
      health: {
    ...
    bookings: bookingRows.map(r => ({
      ...
      source: r.source,
      status: r.status,
      createdAt: parseMySqlUtc(r.created_at) || new Date().toISOString(),
      syncedAt: parseMySqlUtc(r.synced_at),
      note: r.note,
    ...
    pending: pendingRows.map(r => ({
      ...
      notes: r.notes,
      createdAt: parseMySqlUtc(r.created_at) || new Date().toISOString(),
      resolvedAt: parseMySqlUtc(r.resolved_at),
    })),
  };
}
...
async function writeMysqlStore(store: Store) {
  ...
  for (const t of store.theatres) {
    await db.query(`INSERT INTO theatres (
      ...
      toMySqlUtc(t.lastHeartbeatAt),
      toMySqlUtc(t.updatedAt),
      t.health.appHealthy,t.health.dbHealthy,t.health.bookingApiHealthy,
    ]);
  }
  for (const b of store.bookings) {
    await db.query(`INSERT INTO bookings (
      ...
      b.showTime || null,b.bookingMode || null,b.source,b.status,toMySqlUtc(b.createdAt),
      toMySqlUtc(b.syncedAt),b.note || null,b.requestIp || null,b.sourceLabel || null,
    ]);
  }
  for (const p of store.pending) {
    await db.query(`INSERT INTO pending_transactions (
      ...
      toMySqlUtc(p.createdAt),toMySqlUtc(p.resolvedAt),
    ]);
  }
}
